...
- **memory 是软建议**(prompt 拼接,LLM 可推翻);硬访问控制走 §3.8 ACL 层。两者职责正交:memory 管"怎么答",ACL 管"谁能看到内容"。

### 3.8 Topic ACL(M8)— 话题级硬访问控制

procedural memory 是 prompt 层软指令,实测会被反诘 / 换皮关键词("螃蟹是哥对周婷的称谓")绕过。涉及人物隐私 / 内部花名 / 私下评价的话题需要硬访问控制 — LLM 看不到原文,无从泄漏。

**设计原则**:
- 不靠关键词穷举(隐喻无法列举完全)
- 内容入库时 LLM 语义判定 topic → 落 `acl_topic_id`;5 类候选表冗余继承,retrieve 出口纯列过滤,O(1) 不 join
- 白名单走 `meta/policies/topic_acl.yaml`,owner-only 修改靠 git PR + repo 权限,鉴权外置,bot 代码不写白名单

**四道闸**(任一拦截即生效):

| 闸 | 位置 | 作用 |
|---|---|---|
| 1. retrieve 出口 | `helper/ask/retrieve.py` | asker 不在 topic.allowed_domains → 带该 topic 标的 hit 全过滤 |
| 2. ask 入口短路 | `helper/ask/runtime.py` | 对 question + chat_context 跑 acl_tag,命中且非白名单 → 直接返 deny_response,**不调主路径 LLM** |
| 3. 出口 scrub | `helper/ask/runtime.py` | 主路径生成的 answer 文本若含 yaml 黑词("刘佳翔") → 整段替换为 deny_response。兜底防 LLM 凭参数知识脑补 |
| 4. chat_context 过滤 | `helper/storage/raw_store.py` | 拼群历史时按 asker 过滤,带 acl 标的 raw 直接跳过。防白名单连续聊后非白名单穿插提问拿到敏感上下文 |

**与 memory 的关系**:memory 答题口径(如"哥别复述身份")保留 — 白名单内的人问起仍按这条走;非白名单一律走 ACL 拒。

详见 `bot/helper/acl/` 模块和 §8.6 策略外置纪律(topic_acl.yaml 与 knowledge_policy / llm_routing 同位三件套)。
...
- 策略外置三件套: `meta/policies/{knowledge_policy, llm_routing, topic_acl}.yaml` 同位放 spec git repo,改策略走 PR + git diff,bot 代码不写阈值/路由/白名单
...
/var/lib/helper/ (服务器) 或 ./var/helper/ (本地开发)
  ...
  │   ├── facts/
  │   ├── cases/
  │   └── meta/policies/       ← knowledge_policy.yaml + llm_routing.yaml + topic_acl.yaml
