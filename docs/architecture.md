...
- 鉴权外置 — 白名单走 `meta/policies/topic_acl.yaml`(spec git repo,defaults/ 兜底),owner 改 yaml + 重启,代码不写白名单。

**4 道闸**(数据流防漏 3 道 + 模型幻觉兜底 1 道,任一命中即生效)。具体位置 / 顺序 / 行为见 [`runtime.md` §2.7](runtime.md)。

代码: `bot/helper/acl/`(policy + tagger)。yaml 与 `knowledge_policy / llm_routing` 同位,详见 §8.5 策略外置三件套。
