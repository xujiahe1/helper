...
| **L1** | **抽成 0..N 条知识原子**,二分:`section`(语义独立单元,原文保留) / `decision`(为什么这么做:场景→选项→选择→理由) | 产品自动 |
| **L2** | **聚类成模式**:`decision` 按语义向量聚簇成 `SpecTopic`(centroid + decision_count)→ 满足触发判据时产出候选 spec | 产品自动 + 专家点头 |
| **L3** | **边界标注**:为每条规则补上反例和不适用条件 | **产品主动追问** + 专家答 |
| **L4** | **可执行化**:编译为 agent 可直接消费的知识库(specs/) | 产品自动 |
| **L5** | **回流**:agent 跑历史 case → 不一致点 → 回流到 L2/L3 | 产品自动 + 专家 review |

> L1 不预设"一段聊天 1 条 / 一篇 SOP N 条"。文本里有多少值得沉淀的原子,就抽多少。
> 历史上 L1 曾有 5 类细分(fact / case / concept / relation),实测发现 LLM 在边界判定上不稳,把成体系的内容(表格 / 清单 / 映射)拆碎反而丢集合性,M11 起退役为 section + decision 二分。
> 决策五元组 `{情境, 信号, 权衡, 决策, 原因}` 是 `decision` 的字段。
...
**Bot-routing 分支(M6)**: ask 答题前先 LLM judge 当前问题是否命中"涉及 X 去问外部 bot Y"这类 directive;命中则产出 `RouteRequest`(而非 `Answer`),走 [`bot/helper/im/bot_routing.py`](../bot/helper/im/bot_routing.py) 把问题代发给外部 bot,等回执后回贴并加 `@asker 已咨询 @via:` 前缀,LLM 把对方原文重写成 markdown card。详见 §3.6。
...
### 信息修正统一路径(L1 二分 + procedural memory 全打)

> 所有修正都走同一条流水线 — **不区分 spec/decision/section/memory**。
> 任意类型的"新输入和既有不一致"都是冲突,都进 inbox,都由 owner 在
> 「采纳 / 保留 / 都留」三选项里裁决。

```
新 raw → L1 二分(section/decision) + memory_extract(并行)
                              ↓
            sink._run_consumers 自动跑 conflict.detector
                              ↓
        ┌─────────────────────┴─────────────────────┐
                  decision-vs-spec, memory
                  ─────────────────────────
                  全部统一走 LLM judge
...
落地纪律:
- **conflict_log 不分表** — `target_type ∈ {spec, memory}` 字段决定后续处置
- **spec + memory 全过 LLM judge**: 权威/newest-wins/coexist 可自动落定,judge 模型 sonnet
...
### 3.5 Spec 沉淀触发判据(L2→L4 之间的"什么时候真去 draft 一条 spec")

`SpecTopic` 已经按语义把同向 decision 归到了一起,但**不是攒到 N 条就自动 draft**。
触发判据 3 分支(`bot/helper/specgen/draft.py::draft_spec_from_cluster`,任一满足就触发):

| 分支 | 判据 | 含义 |
|---|---|---|
| **普适** | LLM `spec_universal_check` 判 `is_universal=true` | 题面里出现"以后都"、"统一这样"、"通用规则"等显式普适标记 → 1 条 decision 即可 draft |
| **饱和** | `decision_count >= 3` | 同一语义簇被反复说 ≥ 3 次,即便每次都不普适也兜底 draft |
| **静默** | 簇里最老 raw ≥ 90 天前 + 簇从未被 SpecCandidate 引用过 | 老 raw 仍未沉淀过 → 静默触发,避免长尾判断永远没机会变 spec |

避坑:静默分支会 check 簇是否已被某条 SpecCandidate 引用过(无论 superseded 与否),已引用就跳过 — 避免反复打扰 owner。
冷却:成功 draft 后写 `spec_topics.last_promoted_at`,30 天内同簇 `scan_topics_for_draft` 不再返回。

### 3.5.1 Approved spec 防覆写(改动 5)

L2 重新聚簇 / L4 编译时,**已被 owner 在周报上 approve 过的 SpecCandidate 不会被新 draft 覆盖**。
新 draft 即便落到同 slug 也会 fork 出新行,owner 看周报决定是否替换 — 防止刚批准的规约被下一轮聚类静默改写。
...
3. 收到 Y 的回执后,**前缀** `@asker 已咨询 @Y:` + **LLM 转述**: 抽对方原文 → `restate_bot_reply` task 重写成 markdown → 用 card 内嵌 markdown 组件发出。不原样转发 — 外部 bot 的 form/button 组件在 helper 应用下过不了 Wave 校验(retcode 10401069);扁平 i18n_text 也会丢表格/换行
...
- 现有 L1 二分(section/decision)全是描述客观世界的 **semantic memory**。
...
- 命中路径(三路并集任一命中即拼,见 `bot/helper/memory/lookup.py::directives_for_ask`):
  1. **scope=global** 一律拼
  2. **题面 entity 命中** — retrieve 在 question 文本里抓到的 entity 名 → 拼对应 scope=entity 的 directive
  3. **asker 自身的 canonical 名** — `asker_domain` 通过 `entity_alias` 反查中文名后加进 entity_refs;alias 表无记录时走 `bot/helper/im/wave_user.ensure_alias_for_domain()` 调 Wave OpenAPI `/openapi/contact/v1/users/get` lazy 拉一次落 source='auto' 缓存,失败不阻塞 ask。这条解决"X 本人来 @bot 问别的问题,但题面里没有他的名字 → scope=entity:<X> 的 directive 漏注"。
- 同义实体表 `entity_alias`:把"小猫老师/周婷"、"yuqing.chen/陈雨晴/牛姐"这种**同一对象不同名字**归到 canonical 主名,落 Memory.scope_ref 之前先过 `resolve_alias()`。source 优先级 manual > auto > reverted(owner 在周报否决"疑似同义"时标 reverted,防 auto 路径反复触发)。
- Memory 冲突检测的向量 fallback:同 scope 没字面冲突时,跨 scope 跑余弦 ≥ 0.85 → 落"疑似同义"型 ConflictLog,周报让 owner 决定"采纳=用新覆盖旧 / 保留=否决 / 都留"。
...
│  ral, 软)    │              │  语言定时)   │                 │
├──────────────┼──────────────┼──────────────┴────────────────┤
│ ACL Layer    │ Model Router │ ⏳ Agent Surface (M10)        │
│ (M8 话题级硬 │              │   内置 Claude Agent SDK,      │
│  过滤,4 闸) │              │   走 Athenai,KB 注入起手     │
│              │              │   prompt,详见 runtime.md §4   │
└──────────────┴──────────────┴────────────────────────────────┘
       ↓                                              ↑
┌──────────────────────────────────────────────────────────────┐
...
> ⚠️ **状态说明 (M11 起)**:本节讨论的 fact / case / concept (entity) / relation 4 类候选表与"ontology 涌现"机制已退役。当前实施的 L1 是 section + decision 二分(详见 §2 / §3.4)。本节内容作为**未来设想**保留,不代表当前代码行为。涉及的 facts/ / cases/ / ontology/ 目录、entity 晋升流程、知识政策 yaml 当前未在产品中体现。
...
新策略上线前可以双跑评估: 同一批新 raw input 同时按 v1 / v2 策略各自走一遍,产出两份候选 entity 列表,人工对比哪个更合理再切换。
...
/var/lib/helper/ (服务器) 或 ./var/helper/ (本地开发)
  ...
  └── (⏳ M10) agent-workdir/<task_id>/   ← Agent Surface 实装后,内置 Claude Agent SDK 工作目录,产物落这,TTL 默认 7 天
```
...
| L1 派生 | `l1_results` / `l1_items`(section/decision 二分,decision 带 `embedding` + `topic_id`)/ `fact_candidates` / `case_candidates` / `relation_candidates` / `entity_candidates` / `spec_candidates`(带 `topic_id` 反指,全部带 `acl_topic_id` 列冗余继承自 raw,M8) |
...
| Memory(M5) | `memories`(scope_type / scope_ref / directive / superseded_at)/ `entity_alias`(name / canonical / source ∈ {manual, auto, reverted}) |
| Spec 聚类 | `spec_topics`(centroid 1024-d fp16 / decision_count / last_promoted_at)— L2 语义聚簇,decision 写入时按余弦 ≥ 0.78 归簇,daily 03:00 `spec_topic_scan` 满足触发判据的簇产出 SpecCandidate |
| Routing(M6) | `pending_routings`(target_app_id / via_label / tracker_card_msg_id / consumed_at) |
| 会话控制 | `chat_context_cutoffs`(scope_key / cutoff_raw_id) — `/clear` 命令钉的"上下文加载起点", 老 raw 不再进 prompt 但物理保留 |
