...
def index_fact(sess: Session, slug: str) -> int | None:
    """fact_candidate 写入 / supersede 时同步。语义召回 ↔ 词面召回 RRF 融合。"""
    ...
    content = "\n".join(p for p in parts if p)
    return upsert(sess, kind="fact", ref=slug, content=content)


def index_case(sess: Session, slug: str) -> int | None:
    ...
    content = "\n".join(p for p in parts if p)
    return upsert(sess, kind="case", ref=slug, content=content)


def index_relation(sess: Session, slug: str) -> int | None:
    ...
    content = "\n".join(p for p in parts if p)
    return upsert(sess, kind="relation", ref=slug, content=content)
