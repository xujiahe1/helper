...
归属人(谁说的)统一靠 raw_inputs.author_domain 反查 — 抽取阶段不存 speaker 字段,
答题时 bot 也不会主动复述"xxx 说"。原文里若有"X 说 ..."的转述, body 原样保留即可。
"""
...
2. decision — 仅当一段明确是「为什么这么做」(场景→选项→选择→理由)时
   字段:
     ...
     choice:     最终选择
     rationale:  理由
     source_raw_ids / primary_raw_id(群聊场景填,否则可空)
...
B) 群聊 @bot 触发,带 ## 上下文 + ## 主消息 两块 —
   ...
   群聊场景下 decision 必填 source_raw_ids(主 + 上下文)+ primary_raw_id。

【作者归属(重要)】
输入若标了 `[作者: <domain>]` 或 `## 当前消息(作者: <domain>)`,这是当前消息发信人。
内容**默认归属该作者**,不要另抽 speaker 字段、不要从内容里推断别人作为说话人。
- 自陈("张三是好人") → body 就是原文,作者已在 raw 元信息里
- 转述("李四说张三是好人") → body 保留原文,作者仍是当前发信人(他在转述)
  转述里的"X 说"是 body 内容的一部分, **不**要因此把 entities/topics/title 反向归到 X。
不要在 title / body / signals / rationale 里主动复述"xxx 说" — 除非原文本身就这么写。
    ...
    *,
    ...
    for i, chunk in enumerate(chunks):
        chunk_prompt = _build_user_prompt(
            chunk, context=None, primary_raw_id=None, primary_speaker=primary_speaker,
            user_instruction=user_instruction,
        )
