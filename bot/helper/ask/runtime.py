...
而当前问题命中了那类场景, 不要自己回答, 直接输出:
{
  "action": "route",
  "target_app_id": "cli_xxx",
  "via_label": "<bot 名字, 如 tachi>"
}
判断从严: 必须 memory 里有明确的"路由给 bot X"指令, 且当前问题的话题/关键词命中 memory 里写的场景。
...
def ask(
    ...
    *,
    ...
    # 路由分支: LLM 判定该转外部 bot, 不走 helper 自答
    if str(data.get("action", "")).lower() == "route":
        target_app_id = str(data.get("target_app_id", "")).strip()
        via_label = str(data.get("via_label", "")).strip()
        if target_app_id:
            return RouteRequest(
        ...
        log.warning("ask LLM returned action=route but missing target_app_id, fallback to answer")
